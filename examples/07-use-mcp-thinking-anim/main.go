...
func main() {
	...
	streamCallback := func(thinkingCtrl *ui.ThinkingController) func(string) error {
		...
		return func(content string) error {
			// If thinking controller is provided, update its content
	...
	thinkingCtrl.Start(ui.Blue, "Bob is thinking...")

	finishReason, _, _, err := toolAgent.DetectToolCallsStream(messages, executeFn, streamCallback(thinkingCtrl))
	if err != nil {
		panic(err)
	}
